...
const DarkYahtzee = () => {
  ...
  const calculateScore = useCallback((category) => {
    ...
    switch (category) {
      ...
      case 'threeOfAKind':
        return Object.values(diceCounts).some(count => count >= 3) ? sumDice() : 0;
      case 'fourOfAKind':
        return Object.values(diceCounts).some(count => count >= 4) ? sumDice() : 0;
      case 'fullHouse':
        return hasFullHouse(diceCounts) ? 25 : 0;
      case 'smallStraight':
        return hasSmallStraight(diceCounts) ? 30 : 0;
      case 'largeStraight':
        return hasLargeStraight(diceCounts) ? 40 : 0;
      case 'yahtzee':
        return Object.values(diceCounts).some(count => count === 5) ? 50 : 0;
      case 'chance':
        return sumDice();
      default:
        return 0;
  ...
  const scoreCategory = (category) => {
    if (
      ...
      setPlayerScores(prev => {
        ...
        // Set the chosen category score
        playerData.scores[category] = score;

        // Now recalc upper, bonus, total, etc.
        let upper = ['ones', 'twos', 'threes', 'fours', 'fives', 'sixes'].reduce(
          (sum, cat) => sum + (playerData.scores[cat] ?? 0),
          0
        );
        ...
        let lower = [
          ...
          'yahtzee',
          'chance'
        ].reduce((sum, cat) => sum + (playerData.scores[cat] ?? 0), 0);

        playerData.totalScore = upper + bonus + playerData.yahtzeeBonus + lower;

        // Put the updated player data back
      ...
      if (players > 1) {
        ...
        setMessage(`Player ${nextPlayer + 1}'s turn. Roll the dice!`);
      } else {
  ...
  const togglePlayers = () => {
    if (!gameOver && currentTurn === 1 && rollsLeft === 3) {
      ...
      setMessage(
        players === 1
          ? '2-Player mode activated! Player 1 starts.'
          : '1-Player mode activated!'
      );
    }
  };
